...
export default function EmployeesFetchSuperAdmin(){
    ...
    const handleEditSubmit = async (e) => {
        ...
        e.preventDefault();
        try {
            const response = await axios.put(`http://localhost:4000/employee/editEmployee/${employee._id}`, editedClient);
            console.log("Client updated:", response.data);
            // Reload clients after edit
            const employeeResponse = await axios.get(`http://localhost:4000/employee/fetchEmployees`);
    ...
    const handleDelete = async (employeeId) => {
        try {
            await axios.delete(`http://localhost:4000/employee/deleteemployee/${employeeId}`);
                            ...
                            <tbody>
                                {employees.map(employee  => (
                                    <tr key={employee._id}>
                                        ...
                                        <td className="border-t px-4 py-2">{employee.salary}</td>

                                        <td className="border-t px-4 py-2">
                                            <button onClick={() => {setIsUpdating(true);handleEdit(employee._id)}} className="bg-blue-500 hover:bg-blue-600 text-white font-bold py-1 px-2 rounded mr-2">Edit</button>
                                    ...
                                    <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                        ...
                                        <div className="mt-2">
                                            ...
                                            <form onSubmit={handleEditSubmit}>
                                                ...
                                                <input type="email" name="email" id="email" className="mt-1 p-2 border border-gray-300 rounded-md" defaultValue={editedClient.email} onChange={handleInputChange} />
                                                <label htmlFor="email" className="block text-sm font-medium text-gray-700 mt-4">Password</label>
                                                <input type="password" name="password" id="password" className="mt-1 p-2 border border-gray-300 rounded-md" value={editedClient.password} onChange={handleInputChange}/>                                                <div className="mt-4">
